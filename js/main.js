...
function initNavigation() {
    ...
    // Active link highlighting (homepage scroll only)
    const navSectionIds = ['home', 'about', 'services', 'pricing', 'workshops', 'approach', 'contact'];
...
function initSmoothScrolling() {
    ...
    links.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const isSamePageLink = href.startsWith('#') || (href.startsWith('index.html#') && onIndexPage);
...
function initMobileSubmitButton() {
    ...
    function ensureButtonVisibility() {
        if (window.innerWidth <= 768) {
            ...
            submitBtn.style.backgroundColor = '#6B7D6D';
...
// Web3Forms lightweight handler for new booking/workshop forms
(function(){
  document.querySelectorAll('form[action="https://api.web3forms.com/submit"]').forEach(function(form){
    if (form.dataset.web3Handled) return;
    form.dataset.web3Handled = 'true';
    form.addEventListener('submit', async function(e){
      ...
      try {
        ...
        if (data.success) { form.reset(); status.textContent = 'Thank you — your message has been received. We’ll get back to you soon.'; }
        else { status.textContent = 'Sorry, something went wrong. Please try again.'; }
      } catch(err) { status.textContent = 'Sorry, something went wrong. Please try again.'; }
    });
  });
})();
